...
export default function ChatComponent({
  ...
  const [selectedAnimal, setSelectedAnimal] = useState(
    typeof selected === "string" ? selected : "cat"
  );
  const [isOpen, setIsOpen] = useState(false);
  const [previousMessages, setPreviousMessages] = useState<any[]>([]);
  const [previousSelectedAnimal, setPreviousSelectedAnimal] =
    useState<string>("");
  ...
  return (
    <div className="bg-gradient-to-br from-darkgray to-[#2A2A2A] h-screen flex flex-row">
      {isOpen && (
        <SummaryModal summary={summary} onClose={() => setIsOpen(false)} />
      )}
      <Logo mode="light" />
          ...
          <div className="flex gap-4 items-center">
            <img
              ...
              className={`h-12 w-12 cursor-pointer outline outline-white outline-offset-2 transition-all duration-75 ${
                selectedAnimal === "cat"
                  ? "outline-2 opacity-100"
                  : "outline-0 opacity-20"
              }`}
              onClick={() =>
                !isConnecting && !sessionStarted && setSelectedAnimal("cat")
              }
            />
            <img
              src="bee.jpg"
              ...
              onClick={() =>
                !isConnecting && !sessionStarted && setSelectedAnimal("bee")
              }
            />
            <img
              src="frog.png"
              ...
              onClick={() =>
                !isConnecting && !sessionStarted && setSelectedAnimal("frog")
              }
            />
          </div>
          <button
      ...
      <div className="bg-neutral-900 w-1/4 flex flex-col mc-container">
        <p className="text-center border-b-2 p-4 font-bold text-lg text-white border-neutral-600">
          Session Chat
        </p>
              ...
              if (
                ...
                return (
                  <ChatMessage
                    ...
                    author={
                      msg.type == "user_message" ? "user" : selectedAnimal
                    }
                    time={msg.receivedAt}
                  />
                );
            ...
            <>
              ...
              {previousMessages.length > 4 &&
                !isConnecting && ( // Only show if not connecting
                  <>
                    <p className="text-neutral-200 font-medium italic text-center">
