...
const ChatMessage = ({ message, author, time }: Message) => {
	return (
		<div className="flex gap-3 w-full px-2 sm:px-4">
			{/* Avatar */}
			<img src={author === 'user' ? 'steve.png' : author === 'cat' ? 'cat-pfp.svg' : author === 'bee' ? 'bee.jpg' : 'frog.png'} className="w-10 h-10 bg-neutral-300 shrink-0 rounded-full" alt={`${author} avatar`} />

			{/* Message Block */}
			<div className="flex flex-col flex-1 min-w-0">
				<div className="flex gap-3 items-center">
					<p className="font-bold text-white">{author === 'user' ? 'You' : author === 'cat' ? 'Meowtivator' : author === 'bee' ? 'Therabee' : 'Froglosopher'}</p>
